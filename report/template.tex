...
\subsection{Анализ}
Далее необходимо найти такое $C$, что $\sigma^2n\log n + Cn\log n \in CI$ (формула (\ref{eq:Var})). И такое $C$ нашлось! (в отличие от мат. ожидания): $C \in [32.622271995067116, 35.62588847912787]$. Для определенности, пусть $C = 34.124080237097495$ (середина). Вот используемый код:
\begin{lstlisting}[language=Python]
def find_C_Var():
...
где
\begin{itemize}
	\item $\Delta$ -- желаемая точность ($P(|\overline{X}-E|) \leq \Delta = 1 - \alpha$ -- необходимо, чтобы экспериментальное среднее отклонялось от истинного не более чем на величину $\Delta$ с заданной вероятностью $1 - \alpha$).
